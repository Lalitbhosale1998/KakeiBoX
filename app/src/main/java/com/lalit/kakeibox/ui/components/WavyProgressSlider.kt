...
/**
 * A custom slider with a wave effect that moves along the active progress track.
 * The wave flattens when the user interacts with the slider or when animation is disabled.
 * The thumb morphs from a circle to a vertical line on user interaction.
 */
...
fun WavyProgressSlider(
    ...
    waveAmplitudeWhenActive: Dp = 3.dp,
    waveLength: Dp = 80.dp,
    ...
    // The wave is drawn only when eligible, animating, and the user is not dragging
    val shouldShowWave = isWaveEligible && shouldAnimate && !isInteracting

    val animatedWaveAmplitude by animateDpAsState(
        targetValue = if (shouldShowWave) waveAmplitudeWhenActive else 0.dp,
