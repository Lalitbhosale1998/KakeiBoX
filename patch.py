...
with open('app/src/main/java/com/lalit/kakeibox/ui/theme/KakeiboXApp.kt', 'r', encoding='utf-8') as f:
    content = f.read()

# We want to replace the block starting at FULL_WIDTH up to SPLIT
pattern = re.compile(r'                    if \(themeSettings\.navBarStyle == NavBarStyle\.FULL_WIDTH\) \{.*?(?=                    if \(themeSettings\.navBarStyle == NavBarStyle\.SPLIT\) \{)', re.DOTALL)

replacement = '''                    if (themeSettings.navBarStyle == NavBarStyle.FULL_WIDTH) {
...
with open('app/src/main/java/com/lalit/kakeibox/ui/theme/KakeiboXApp.kt', 'w', encoding='utf-8') as f:
    f.write(new_content)
